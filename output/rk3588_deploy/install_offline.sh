...
' 2>/dev/null; then
    ...
    # ── 前置检查: wheelhouse 得真拷全 (漏拷时 pip 只会报 No matching distribution, 看不出来) ──
    if [ ! -d "$WHL" ] || ! ls "$WHL"/rknn_toolkit_lite2-*.whl >/dev/null 2>&1; then
        echo "!! $WHL 缺失或没拷全 (缺 rknn_toolkit_lite2-*.whl)"
        echo "!! 在 PC 上整包重拷:  scp -r output/rk3588_deploy/ root@<板子IP>:/root/"
        exit 1
    fi
